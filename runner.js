...
if (localbrowser.storage.getItem(currentPageUrl) === null) {
    // Page is not cached, so let's cache it.
    // Fetch the page content and store it in localbrowser.storage.
    fetch(currentPageUrl)
        .then(response => response.text())
        .then(pageContent => {
            localbrowser.storage.setItem(currentPageUrl, pageContent);
        })
        .catch(error => {
    ...
    const cachedPage = localbrowser.storage.getItem(currentPageUrl);
...
function setDefaultValues() {
    ...
    browser.storage.local.get(['theme'], function(result) {
        ...
        if (themeType === undefined || themeType === null) {
            ...
            browser.storage.local.set({ theme: themeType }, function() {
